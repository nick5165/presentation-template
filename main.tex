...
\usepackage{paratype} % Идеально: заменяет и текст, и символы на векторные (уберет генерацию шрифтов)

\usepackage{cmap}
\usepackage{xspace}
\usepackage[normalem]{ulem}

% graphicx и hyperref удалены из преамбулы, так как beamer загружает их сам автоматически.

\beamertemplatenavigationsymbolsempty
\setbeamertemplate{footline}[frame number]
\usecolortheme{seahorse}

% Цвет для заголовков таблиц (используем формат RGB, он самый надежный)
\definecolor{tabgreen}{RGB}{146, 208, 80}

% ОГРОМНЫЙ БЛОК с греческими буквами УДАЛЕН.
% Именно он вызывал критическую ошибку и падение (exit code 1). 
% В презентации формул нет, поэтому он не нужен.
